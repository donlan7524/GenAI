...
def parse_and_append():
    ...
    print(f"🎬 啟動資料庫追加導入管線：讀取 {CSV_FILE} ...")
    ...
    print(f"✅ 解析完成！共偵測到 {len(df)} 篇貼文，留言欄位 {len(comment_cols)} 個。正在以 ON CONFLICT 更新方式追加至資料庫...")
    ...
    for idx, row in df.iterrows():
        try:
            ...
            # 寫入或更新 posts 表 (保留其他表，僅對重複的 post_id 做屬性覆寫)
            cursor.execute("""
    ...
    # 重新計算 daily_summary
    print("📊 正在重新整理每日情緒統計 (daily_summary)...")
    ...
    print(f"🎉 追加導入完畢！成功處理 {post_success} 篇貼文與 {comment_success} 則留言到 {DB_FILE} 中！(跳過空白 ID 貼文: {post_skipped} 篇)")
