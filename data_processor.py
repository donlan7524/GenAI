...
class MLPClassifier:
    ...
    用於過濾無效、過短或灌水的論壇留言
    """
    def __init__(self, input_dim=5, hidden_dim=8, output_dim=1, lr=0.1):
        self.lr = lr
        # 初始化權重與偏置
...
def extract_features(text):
    ...
    return np.array([f_len, f_zh, f_spec, f_garbage, f_en])
...
def export_to_fine_tuning_jsonl(trees, poster_out="poster_dataset.jsonl", commenter_out="commenter_dataset.jsonl"):
    ...
    for tree in trees:
        ...
        # 2. 遍歷對話樹，產生 Commenter 留言回覆數據
        # 我們使用 DFS 遍歷對話樹中的所有分支，將祖先節點的發文/留言作為 context，當前節點留言作為 output
        def dfs_traverse(node, path_context):
            # path_context: List of strings representing the dialogue history
            # For each child, their training pair is (path_context, child.content)
    ...
    print(f"📤 發文微調數據已寫入：{poster_out} (共 {len(poster_records)} 筆)")
